...
public class TaskDetailFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        taskList.add(new KeyValueInfo("注册资本（中方）", mEntity.getRegMoneyNational() == 0.0 ? "--" : mEntity.getRegMoneyNational() + " 万元"));
        if (mEntity.getIsForeign() == 1) {
            taskList.add(new KeyValueInfo("投资主体（外方）", mEntity.getInvestorEntityForeign()));
            taskList.add(new KeyValueInfo("注册资本（外方）", mEntity.getRegMoneyForeign() == 0.0 ? "--" : mEntity.getRegMoneyForeign() + " 万美元"));
        }
        taskList.add(new KeyValueInfo("合同号", mEntity.getContractNum()));
        taskList.add(new KeyValueInfo("合同投资额（内资）", mEntity.getContractAmountNational() == 0.0 ? "--" : mEntity.getContractAmountNational() + " 万元"));
        taskList.add(new KeyValueInfo("到位投资额（内资）", mEntity.getRealAmountNational() == 0.0 ? "--" : mEntity.getRealAmountNational() + " 万元"));
        if (mEntity.getIsForeign() == 1) {
            taskList.add(new KeyValueInfo("合同投资额（外资）", mEntity.getContractAmountForeign() == 0.0 ? "--" : mEntity.getContractAmountForeign() + " 万美元"));
            taskList.add(new KeyValueInfo("到位投资额（外资）", mEntity.getRealAmountForeign() == 0.0 ? "--" : mEntity.getRealAmountForeign() + " 万美元"));
            taskList.add(new KeyValueInfo("合同投资总额", (mEntity.getContractAmountNational() + mEntity.getContractAmountForeign() * 6.9495) == 0.0 ? "--" : (mEntity.getContractAmountNational() + mEntity.getContractAmountForeign() * 6.9495) + " 万元"));
            taskList.add(new KeyValueInfo("到位投资总额", (mEntity.getRealAmountNational() + mEntity.getRealAmountForeign() * 6.9495) == 0.0 ? "--" : (mEntity.getRealAmountNational() + mEntity.getRealAmountForeign() * 6.9495) + " 万元"));
        }
        taskList.add(new KeyValueInfo("预计达产产值", mEntity.getExpectedOutput() == 0.0 ? "--" : mEntity.getExpectedOutput() + " 亿元"));
        taskList.add(new KeyValueInfo("开工日期", DateUtil.getDateFromMillis(mEntity.getRealStartDate())));
        taskList.add(new KeyValueInfo("税收", mEntity.getTaxTotal() == 0.0 ? "--" : mEntity.getTaxTotal() + " 万元"));
        ...
        if (mEntity.getIsLandProj() == 1)
            taskList.add(new KeyValueInfo("项目占地面积", mEntity.getRealAmountForeign() == 0.0 ? "--" : mEntity.getRealAmountForeign() + " 亩"));
        else
            taskList.add(new KeyValueInfo("楼宇面积", mEntity.getBuildingArea() == 0.0 ? "--" : mEntity.getBuildingArea() + " 平米"));
        taskList.add(new KeyValueInfo("项目地址", mEntity.getProjAddr()));
        taskList.add(new KeyValueInfo("地理位置", "经度：" + (mEntity.getLongitude() == 0.0 ? "--" : mEntity.getLongitude()) + " 纬度：" + (mEntity.getLatitude() == 0.0 ? "--" : mEntity.getLatitude())));
